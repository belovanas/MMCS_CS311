...
%token BEGIN END CYCLE ASSIGN ASSIGNPLUS ASSIGNMINUS ASSIGNMULT SEMICOLON WRITE VAR PLUS MINUS MULT DIV MOD LPAREN RPAREN COLUMN IF THEN ELSE WHILE DO REPEAT UNTIL COLON GOTO
%token <iVal> INUM 
%token <dVal> RNUM 
%token <sVal> ID

%type <eVal> expr ident T F 
%type <stVal> statement assign block cycle write empty var varlist if while repeat label goto
...
label	: ident COLON statement 
		{
			$$ = new LabelNode($1 as IdNode, $3); 
		}
		;

goto	: GOTO ident
		{
			$$ = new GotoNode($2 as IdNode); 
		}
		;
